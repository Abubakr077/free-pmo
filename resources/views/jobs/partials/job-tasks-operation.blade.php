            ...
            <div class="panel-body">
                <div class="row">
                    @if(auth()->user()->hasRole('supervisor'))
                        <div class="col-sm-3">{!! FormField::text('name') !!}</div>
                    @else
                        <div class="col-sm-3">{!! FormField::text('name',['readonly' => 'true']) !!}</div>
                    @endif
                    <div class="col-sm-3">
                        @can('updateSupervisor',$editableTask)
                            @if ($editableTask->progress < 100)
                                {!! FormField::select('status_id', [
                    1 => 'Pending',
                    2 => 'Accepted',
                    3 => 'Rejected',
                ], ['label' => __('Status'), 'value' => $editableTask->status_id]) !!}
                            @endif
                        @endcan
                    </div>
                    @can('updateSupervisor',$editableTask)
                        ...
                        @forelse($files as $key => $file)
                            <tr id="{{ $file->id }}">
                                ...
                                <td>
                                    ...
                                    <div class="text-info small">{{ $file->hash }}</div>
                                </td>
